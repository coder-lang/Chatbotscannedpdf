...
FORCE_FROM_LANG = ""  # auto-detect: handles both Gujarati and Devanagari OCR output
...
def _translate_chunk(texts: List[str], force_from_lang: str = "gu") -> List[str]:
    ...
    try:
        ...
        # Print status so we can see if API call is reaching server
        print(f"[Translator] HTTP {resp.status_code} for {len(texts)} texts")
        if resp.status_code != 200:
            print(f"[Translator] ERROR response: {resp.text[:300]}")
            return texts
        data = resp.json()
        out = []
        for i, item in enumerate(data):
            if isinstance(item, dict) and "translations" in item and item["translations"]:
                translated = item["translations"][0].get("text", texts[i])
                out.append(translated)
            else:
                print(f"[Translator] Unexpected item format: {item}")
                out.append(texts[i])
        print(f"[Translator] Successfully translated {len(out)} texts")
        return out
...
def extract_text_per_page(pdf_path: Path) -> List[Tuple[int, str]]:
    ...
    try:
        with pdf_path.open("rb") as f:
            poller = client.begin_analyze_document(
                "prebuilt-read",
                f,
                locale="gu"  # Force Gujarati — prevents misread as Devanagari/Hindi
            )
    ...
    for idx, page in enumerate(result.pages or []):
        ...
        print(f"[DEBUG] Page {idx+1}: {len(lines)} lines extracted from OCR")
        if lines:
            print(f"[DEBUG] Sample line: {lines[0][:80]}")
            print(f"[DEBUG] Is Gujarati: {is_gujarati(lines[0])}")
        ...
        print(f"[DEBUG] Page {idx+1}: {len(translated_lines)} lines after translation")
        if translated_lines:
            print(f"[DEBUG] Sample translated: {translated_lines[0][:80]}")
