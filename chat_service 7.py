...
# ── Query translator ──────────────────────────────────────────────────────────
def translate_query_to_gujarati(query: str) -> str:
    """
    Translate English query to Gujarati so vector search matches
    Gujarati document chunks more accurately.
    If query is already in Gujarati, returns as-is.
    """
    try:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are a translator. If the text is in English, translate it to Gujarati. "
                        "If it is already in Gujarati, return it unchanged. "
        ...
        if translated and translated != query:
            print(f"[chat_service] Query translated to Gujarati: {translated}")
        return translated
    except Exception as e:
        print(f"[chat_service] Translation failed, using original query: {e}")
        return query
...
def chat(
    ...
    # Step 2 — Translate query to Gujarati for better vector match with Gujarati chunks
    search_query = translate_query_to_gujarati(user_message)

    # Step 3 — Vector search using translated query — always fetch 15 chunks
    fetch_k = 15
    ...
    if query_years:
        ...
        augmented_query = (
            f"{user_message}\n\n"
            f"[No specific year was requested. Use the LATEST year available "
            f"in the Context and clearly mention which year you are answering from.]"
        )
    ...
    # Deduplicate citations — keep max 3 unique sources
    seen = []
    for c in citations:
        if c not in seen:
            seen.append(c)
