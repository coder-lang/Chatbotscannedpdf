...
def chat(
    ...
    # Step 2 — Vector search — always fetch 15 chunks
    fetch_k = 15
    ...
    # Step 4 — If no year in query, detect latest year from retrieved chunks
    if not query_years:
        all_years_in_chunks = []
        for c in chunks:
            found = re.findall(r"\b20\d{2}\b", c.get("text", ""))
            all_years_in_chunks.extend(found)
        if all_years_in_chunks:
            latest_year = max(set(all_years_in_chunks))
            query_years = [latest_year]
            print(f"[chat_service] No year in query — defaulting to latest year found in chunks: {latest_year}")

    # Step 5 — Filter chunks by year
    ...
    # Step 7 — Augment query with year reminder
