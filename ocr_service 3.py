...
def get_tables_by_page(
    ...
    try:
        with pdf_path.open("rb") as f:
            layout = client.begin_analyze_document(
                "prebuilt-layout",
                analyze_request=f,
                content_type="application/octet-stream"
            ).result()
...
def process_all_pdfs() -> None:
    """
    One-time script. OCRs all PDFs → writes combined_output.txt.
    Run via: python scripts/ingest_pdfs.py
    """
    client    = _make_client()
    ...
    if not input_dir.exists():
        ...
        print("Make sure you run from the project ROOT folder.")
        print("Command: python scripts/ingest_pdfs.py\n")
        sys.exit(1)
    ...
    if not pdf_files:
        ...
        print("Copy your PDF files into the input_pdfs/ folder.\n")
        sys.exit(1)
